...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = [
                ...
                'labelsPadding' => $this->getLabelPadding($tableSchema),
                'enumLabels' => $this->generateEnumLabels($tableSchema),
            ...
            // query : disabled creation of new file
        }
    ...
    /**
     * Calculates optimal string padding value for labels.
     * @param \yii\db\TableSchema $table the table schema
     * @return number The calculated string padding
     */
    protected function getLabelPadding($table)
    {
        $pad = 8; // Set the initial pad to 8 spaces
        foreach ($table->columns as $column) {
            $length = strlen($column->name) + 2; // include the commas
            if ($length > $pad) {
                $pad = $length;
            }
        }
    ...
    /**
     * Generates the labels for enum type columns for the specified table.
     * @param \yii\db\TableSchema $table the table schema
     * @return array the generated columns labels (name => label)
     */
    public function generateEnumLabels($table)
    {
        $labels = [];
        foreach ($table->columns as $column) {
            if (is_array($column->enumValues)) {
                $labels[$column->name] = $column->enumValues;
            }
        }
